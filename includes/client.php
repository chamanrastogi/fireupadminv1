<?php
	...
	protected static $db_fields=array('id','name','image','text','status');
...
exit();
    ...
    $rw=self::find_by_id($_GET['id']);
		...
		echo $fo=Forms::text_editor("Text","text",$text,"text",1);
	 ...
	 echo $fo=Forms::form_end();
      ...
      if($_FILES['image']['size']!=0)
		   {
			   $data->image=$data->image_maker($_FILES['image'],1);
      
			}$data->text=$text;
      
	  if($type=='edit')
{ 
if($rw->image!='' &&  $_FILES['image']['size']!=0)
		   {
			   $data->image=$rw->image;
